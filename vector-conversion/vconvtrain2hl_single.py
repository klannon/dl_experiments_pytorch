...
for trial in range(trials):
    run_training(model = model, modelpath = 'v_conv_model_{}_{}nodes_{}'.format(hl1_nodes,hl2_nodes,trial), trainset = train_set, validsize = valid_size, numepochs = n_epochs, batchsize = batch_size, seed = seed)
...
for trial in range(trials):
    ...
    model = torch.load('v_conv_model_{}_{}nodes_{}'.format(hl1_nodes,hl2_nodes,trial))
...
np.save(fname=results_file_1, arr=mean_errors)
np.save(fname=results_file_2, arr=mean_diffs)
np.save(fname=results_file_3, arr=stdevs)
