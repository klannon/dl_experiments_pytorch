...
results_file_1 =outfile_name = '{}_{}to{}nodes_step{}_errors'.format(args.outbase,args.start_node,args.end_node,args.node_interval)
results_file_2 =outfile_name = '{}_{}to{}nodes_step{}_diffs'.format(args.outbase,args.start_node,args.end_node,args.node_interval)
results_file_3 =outfile_name = '{}_{}to{}nodes_step{}_stats'.format(args.outbase,args.start_node,args.end_node,args.node_interval)
...
model_diffs = np.empty([num_models,2])
model_stats = np.empty([num_models,2])
...
for node in nodes:
    ...
    for trial in range(trials):
        ...
        model = torch.load('v_conv_model_{}node_{}'.format(node,trial))
        ...
        stdev_diff_targ = std_diff / std_targ
    ...
    # get mean error, diff, stat for each model
    model_error = np.mean(mean_errors)
    for i in range(len(nodes)):
        model_errors[i][0] = node
        model_errors[i][1] = model_error

    model_diff = np.mean(mean_diffs)
    for i in range(len(nodes)):
        model_diffs[i][0] = node
        model_diffs[i][1] = model_diff

    model_stat = np.mean(stdevs)
    for i in range(len(nodes)):
        model_stats[i][0] = node
        model_stats[i][1] = model_stat

# save results

np.save(fname=results_file_1, arr=model_errors)
np.save(fname=results_file_2, arr=model_diffs)
np.save(fname=results_file_3, arr=model_stats)
