...
parser.add_argument('-o','--out', dest='outbase', metavar='OUTBASE',
                        help='File name base  for model and results (trial name will be appended)')
...
def run_training(model, modelpath, trainset, validsize, numepochs, batchsize, seed):
    ...
    # save results
    np.save(file='{}_{}node_trial{}_err.npy'.format(args.outbase,node,trial), arr=errors)
    np.save(file='{}_{}node_trial{}_diff.npy'.format(args.outbase,node,trial), arr=diffs)
